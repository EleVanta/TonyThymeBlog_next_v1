...
function createClient({ preview = false } = {}) {
  ...
  // Require contentful only when we actually need to create a client. This
  // avoids loading the Contentful SDK at module import time which can cause
  // build-time failures when env vars are missing.
